...
- `_backend_naive.py` adds the `tl.dot` fused-forward baseline and
...
## Post-M13 — `optimized` promoted to a pure-Triton TMA forward (2026-06-24)

**Not a milestone** (both kernel tracks stayed closed at M13 — this is a forward-backend
replacement + re-baseline, not a track reopen). The default `optimized` forward is now a
pure-Triton persistent kernel, and all Gluon was removed from the package. Design of record:
[ARCHITECTURE.md](ARCHITECTURE.md) §5.1. This section keeps the durable evidence; the winding
exploration that produced it is compressed under "The arc" below (the detailed phase-by-phase
notes are archived out of tree).

### The shipped design and its evidence

**Kernel.** `src/sparton/_backend_optimized.py` (op `sparton::optimized_fwd`): a persistent
...
`tl.make_tensor_descriptor` needs `triton.set_allocator` (a process-global ContextVar side-effect
incompatible with CUDA graphs / the caching allocator). That is the concrete reason "host-side
descriptors" is the directive. Allclose to a torch reference (~6e-5) on small / tile-boundary
(`3×17×48×129`) / multi-tile shapes.

**WS needs a single-result reduce (the #1 risk, fired and fixed — durable Triton lesson).**
`tl.range(warp_specialize=True)` failed to compile: Triton 3.7.1's `TritonGPUPartitionScheduling`
asserts `reduceOp.getResults().size() == 1`, and the strict-tie argmax was a 2-result `tl.reduce`
(value+index together). Fix — two single-result reduces: `tmax = tl.max(vals, 0)`;
`cand = tl.where(vals == tmax[None,:], row_idx[:,None], BLOCK_M)`; `targ = tl.min(cand, 0)`.
Bit-identical to the combine (`tl.max` returns an actual element so `vals == tmax` is exact, and
min-of-qualifying-row-indices == the strict-`>` lowest-index tie-break; `torch.equal`-verified
scores+idx). One kernel carries both forms behind a `WARP_SPECIALIZE` constexpr `if`; the dead
branch is trace-time-eliminated, so the WS path compiles with the combine still in source. (This
also bounds anything `tl.argmax` lowers to.)

**Measured, not derived, tile policy.** A self-implemented autotuner caches
`(D, V, dtype, arch) → (tile, ws)` — **not** B/S (the persistent grid fills the GPU at any trip
count; B/S-independence is unit-tested). On a miss it builds the valid candidate set,
correctness-gates each tile against the analytic tile's own output (a same-kernel baseline — no
nested `naive`, no `[B,S,V]` materialization), then measures in two stages: rank the homogeneous
tiles, then compare {top-2} × {homogeneous, WS} (WS tried only where it can win, dropped where it
loses). `SPARTON_OPTIMIZED_AUTOTUNE` default on; off → the small analytic tile. The offline pick
was provably wrong: the winner for `(4,256,768,2048)` is `128×64×64 s4`, not the analytic `64×64`
(the +14% retile lever, now captured automatically). The tile space is self-contained
(`OptimizedTile` / `_CANDIDATE_TILES` / `_DeviceLimits` in the module) — no shared GEMM-policy.

**`do_bench` thermal/order bias (measurement caveat).** On this 575 W RTX 5090, `do_bench` is
biased by clock-boost-on-first-measurement + thermal throttle + order: whichever variant is timed
first looks artificially fast, which once inflated a real ~1% epilogue gap to a reported "7%". The
real gap (bias-controlled: prewarm, discard the first round, rotate order) is ~1% (large) … ~5%
(small-V), homogeneous combine fastest (registers combine 124 / minmax 123, 0 spills). Per
maintainer directive ("thermal is beyond our control") the production tuner calls **plain**
`triton.testing.do_bench` (no prewarm/rotate wrapper; tuning ~9 s → ~3.5 s) — the bias is recorded
as a caveat, not mitigated in code. The re-baseline deltas below far exceed the bias, so they hold.
(Memory: `do-bench-thermal-order-bias`, `investigate-anomalies-before-building`.)

**`D`/`V` constexpr — measured-neutral, kept for structure.** Folds `k_tiles`/`n_col`/the V-tail,
unrolls the K-loop, strength-reduces the persistent `tile_id` div/mod to a constant-divisor
multiply-shift; B/S stay runtime (one compile per (model, tile)). Isolated A/B: homogeneous
constexpr/runtime ∈ {0.995–1.010}, WS ∈ {0.962–1.001} — neutral, matching M12's compute-bound
finding (the div/mod is small next to the matmul). Kept because it is the correct structure and
production compiles once per model.

**Re-baseline vs the previous Gluon `optimized`** (`bench_sparton_baseline.py`, self-tuner on, bf16,
warm; the §5 first reading is shown where it differed from the post-follow-up figure):

| regime | shape | exp/opt | read |
|---|---|---|---|
| large/throughput | 8×512×1024×151936 | **≈0.99** (§5: 1.02) | parity (the single Gluon tile was +7…16% slower) |
| small/serving | 1×128×768×30522 | **≈0.68** (§5: 0.75) | pure-Triton −32% |
| small-V | 8×256×768×1024 | **≈0.65** | pure-Triton −35% |

The measured autotuner closes the large-grid gap (it selects the 128-family tile) and the
persistent + measured tile (with WS where it wins) beats the old Gluon optimized decisively in the
small regimes.

**Small-regime residual is occupancy, not memory.** ncu on the small/serving WS kernel
(`1×128×768×30522`, 59.7 µs): DRAM 39.6% / L2 49% / SM 44% (nothing saturated); 255 reg/thread →
Block-Limit-Registers = 1 → 16.7% theoretical = 16.65% achieved occupancy; top stall Barrier 30.6%
(the WS CTA sync). ⇒ the small-regime lever is **register/occupancy** (a Triton WS register knob —
the auto-WS analog of Gluon's `setmaxnreg`, not currently exposed), and rasterization-swizzle / L2
hints are **large-regime** levers (they target L2 reuse across a big grid; the small regime sits at
DRAM 40% / L2 49%, so they cannot move it). All such levers are unbuilt and profiling-gated.

**Validation.** Full suite **135 passed** (the promotion merged the parallel optimized/experiment
correctness sets and dropped the Gluon-policy tests); optimized shape soak **24/24** (max score err
1.95e-3, index gap 0); compute-sanitizer racecheck 0 / initcheck 0 on `B4 S33 D64 V2048`
(fully-masked row, non-divisible-D mask path), WS on and off; training-smoke hybrid-vs-optimized
parity (fp16 rel diff ~3e-4, bf16 ~2e-3); `torch.compile` fullgraph green. Op schema / saved-tensor
set / shared backward unchanged.

**Availability / default.** Gated at CUDA capability **sm_90+** (Hopper — host-side TMA) plus
importable `triton.tools.tensor_descriptor` (`_backend_runtime.is_optimized_backend_available`);
below the floor default resolution falls back to `hybrid` with a one-time `RuntimeWarning`. This
deliberately narrows the default envelope vs the old Gluon optimized (sm_80+).

### The arc (explored, did not ship)

The pure-Triton kernel above is the convergent design after a sequence of experiments, each kept
off the default path while open and now removed; their detailed phase notes are archived out of
tree. In order:

- **`gl.warp_specialize` viability spike (sm_120).** Closed the M12 §8 "unprobed" gap: the Gluon
  construct compiles and runs around real `mma_v2` (6e-6 vs fp32), incl. `mbarrier.arrive` and
  `worker_num_regs`. Durable lesson: when probing fatal-abort APIs, **classify compile-error vs
  process-abort** (a call-shape bug first masqueraded as a hardware NO-GO). Viability ≠ benefit.
- **Gluon `experiment` backend (Phase 1).** Persistent grid-stride + one hardware-derived tile + a
  single descriptor pair (no policy bank / autotune key). Bit-identical to `naive`; **~7–16%
  slower** than the per-shape-autotuned Gluon `optimized` on the throughput grid (no wave tail to
  recover — M12). Its real edge was host-side/cold-start, not GPU throughput.
- **Portability seam (D4).** A constexpr `MMA_KIND` + three `@gluon.jit` seam helpers isolating the
  `mma_v2` pieces (wgmma/tcgen05 stubbed behind `gl.static_assert`); bit-identical, zero SASS change.
- **Device-side producer/consumer WS (D3).** A hand-rolled `gl.warp_specialize` GEMM (TMA producer
  warp + `mma_v2` consumer, full/empty mbarriers). The pre-registered A/B fired **SHIP** in the
  latency-bound regime (−11% B=1 / −15% small-V vs homogeneous; neutral on the saturated large grid —
  refining, not overturning, M12). Superseded by the pure-Triton `tl.range(warp_specialize=)` tuned
  dimension (whose register/occupancy lever the small-regime profile above identifies as the next
  step).
- **D5 mask-aware early exit (`SKIP_MASKED`).** A CTA-uniform "any active?" branch skipping
  fully-masked s-tiles, provably bit-identical under strict-`>`. **1.86×** on 44%-active ragged
  padding but **+1.2%** full-length overhead — never promoted, and **dropped** from the pure-Triton
  kernel because the data-dependent `if` around the producer loads is incompatible with
  `warp_specialize=True` (breaks the auto-WS `PartitionLoops` pass).
- **Heterogeneous CUDA-core offload.** Explored (an FMA matmul on CUDA cores alongside the tensor
  pipe), not pursued.
