...
## §2 Triton vs. Gluon: when to use which (retrospective)

The decision table Sparton used. **In the end this project chose Triton**: the
pure-Triton `optimized` forward (host-side TMA + `tl.dot` + `tl.range(warp_specialize=)`)
matched the hand-written Gluon kernel at the saturated large shape and beat it in the
serving regime, so Gluon's extra surface (lazy import shim, capability whitelist, manual
mbarrier protocol, a GEMM-policy universe) was not worth its maintenance cost. The table
still captures *when* the trade would favor Gluon.
...
1. **Measure the tile, don't derive it.** An offline resource model picked a tile that
   ...
   self-contained candidate set, correctness-gate each, and time them; cache the winner.
   A derived "optimal" tile is a hypothesis, not a result.
2. **`do_bench` is clock/thermal/order-biased on consumer Blackwell.** On a 575 W RTX
   5090, whichever variant is timed first looks artificially fast — a real ~1% gap once
   read as "7%". Either control for it (prewarm, discard the first round, rotate order)
   or trust only deltas that exceed the bias; do not over-fit a tuner to back-to-back
   timings. (Sparton kept plain `do_bench` and treats sub-few-percent deltas as noise.)
3. **Autotune key ≠ problem shape.** A persistent grid fills the GPU at any trip count,
   so the optimal tile depends on `(D, V, dtype, arch)` — **not** B/S. Keying the cache
   on operand size + dtype + arch (validated by re-measuring at a different `(B, S)`)
   compiles once per model and serves all batch/sequence shapes.
4. **A persistent kernel's per-tile state reset is a correctness invariant, not a
   detail.** One CTA processes many tiles; running max/idx must reset at the *top* of
   each tile or a prior tile's value leaks. Pin it with a `NUM_CTAS=1` test that forces
   many tiles through one CTA.
5. **Host-side TMA descriptors avoid a process-global side-effect.**
   `triton.tools.tensor_descriptor.TensorDescriptor` is driver-filled on the host, so
   `global_scratch_size == 0` and no `triton.set_allocator` is needed; device-side
   `tl.make_tensor_descriptor` sets a process-global allocator ContextVar (incompatible
   with CUDA graphs / the caching allocator). Prefer host-side when the descriptor shape
   is known on the host.
6. **Warp specialization can be a *tuned dimension*, not a hand-rolled kernel.**
   `tl.range(warp_specialize=)` lets the compiler build the producer/consumer split, so
   one kernel carries homogeneous and WS forms behind a constexpr and the tuner picks per
   shape. Constraint discovered: Triton 3.7.1's auto-WS pass requires **single-result**
   reduces, so a 2-result argmax `tl.reduce` must be re-expressed as `tl.max` + masked
   `tl.min` (bit-identical) for the WS arm. WS wins the latency-bound small/serving
   regime and is neutral on the saturated large grid.
7. **"Saturated" is regime-specific, and so are the levers.** The large grid is
   tensor-pipe + L2 bound (M12) — only *doing less work* (e.g. FP4) or cutting L2 traffic
   moves it; the small/serving regime is occupancy-bound by register pressure (16.7%
   occupancy at 255 reg/thread, top stall Barrier). Distinguish SM-active from
   issue-active, and profile the actual regime before reaching for a lever: a large-grid
   fix (swizzle, L2 hints) cannot move a small-regime occupancy wall, and vice versa.

## §5 Common Gluon-specific techniques

> **Historical / reference.** Sparton evaluated and **removed** Gluon from the package
> (the pure-Triton `optimized` forward replaced the Gluon kernel post-M13). These
> techniques are retained as a durable reference for explicit-control kernel work, not a
> description of any shipped Sparton code; the pure-Triton lessons that superseded them
> are in §4.6.
