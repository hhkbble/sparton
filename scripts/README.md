...
from session scratch on 2026-06-11. The built system — backends, platform
facts, profiling methodology, contracts, kernel designs, and validation gates
— is described in [../docs/ARCHITECTURE.md](../docs/ARCHITECTURE.md); the
per-milestone development evidence (M2–M13) in
...
| `ncu_runner.py` | Policy-derived Gluon GEMM launcher for `ncu --launch-skip`/`--launch-count` | see ARCHITECTURE.md §2.5 |
...
| `dump_backward_ir.py` | Backward autotune-selection capture + per-config TTGIR/PTX/SASS dump with a load/atomic census (METHODOLOGY.md §6.1–§6.2): the standing lowering-visibility tool for the split backward; warm-cache cheap, one op call per shape | `env $ENV PYTHONPATH=src python -u scripts/dump_backward_ir.py [--out tests/data/ir_dump] [--shapes dev,corner,query,doc]` |
