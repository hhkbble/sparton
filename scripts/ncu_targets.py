...
Provides ranges for `ncu --nvtx --nvtx-include "<range>/"`:
  ...
  hybrid_fwd/        one hybrid_fwd_op call (all per-tile kernels)
  ...
  hybrid_bwd_direct/ optimized_bwd_op called directly on the main thread
...
scores, idx = sk.hybrid_fwd_op(hidden, embed, bias, mask)
g2 = torch.ones_like(scores)
torch.cuda.synchronize()
torch.cuda.nvtx.range_push("hybrid_bwd_direct")
sk.optimized_bwd_op(g2, scores, idx, hidden, embed, bias, mask)
torch.cuda.nvtx.range_pop()
torch.cuda.synchronize()
